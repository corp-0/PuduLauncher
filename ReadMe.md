## Getting Started with PuduLauncher
...
npm run build
```
This will emit the production bundle directly into `Resources/wwwroot` so it can be embedded and served by the Photino static file server. That folder is ignored in git because it's generated output.

### .NET
The .NET build also triggers the UI build automatically (`npm install` + `npm run build` from `UserInterface/PuduLauncherUi`). You can skip this step by passing `-p:BuildUi=false` to `dotnet build` if you only want the .NET binary.

After doing a production build of the user interface (or letting the .NET build run it), the .NET application can be built in release mode by using the Configuration Manager in Visual Studio or by issuing the following command from the root .NET project folder:
```bash
